...
export const EXTRA_INDEXES_SQL = [
  ...
  sql`CREATE INDEX IF NOT EXISTS problemes_embedding_idx ON problemes USING hnsw (embedding vector_cosine_ops)`,
  sql`CREATE INDEX IF NOT EXISTS pistes_embedding_idx ON pistes USING hnsw (embedding vector_cosine_ops)`,
];
