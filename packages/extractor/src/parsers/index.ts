...
import { parseBimmerforums } from './bimmerforums.js';
import { parsePhpbb } from './phpbb.js';
...
export function parseThread(html: string, url: string): ParsedThread {
  ...
  if (host.endsWith('bimmerforums.com')) return parseBimmerforums(html, url);
  if (PHPBB_HOSTS.some((h) => host.endsWith(h))) return parsePhpbb(html, url, host);
